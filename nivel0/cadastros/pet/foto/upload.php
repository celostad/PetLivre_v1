...
$sql_excl = mysqli_query($connection, "SELECT * FROM `tab_temp_pet` WHERE user_cadastro='$usuario'") or die("Erro ao selecionar   -   Selecionar User_cadastro inicial  SQL");
...
$sql_1 = mysqli_query($connection, "SELECT * FROM `tab_temp_pet` WHERE user_cadastro='$usuario'") or die("erro ao selecionar1 - TELA UPLOAD");

if ($linha = mysqli_fetch_array($sql_1)){

$sql_2 = mysqli_query($connection, "UPDATE `tab_temp_pet` SET caminho_foto ='$caminho' WHERE user_cadastro='$usuario'") or die (mysqli_error($connection));

}else{

$sql_3 = mysqli_query($connection, "INSERT INTO `tab_temp_pet` (`caminho_foto`, `user_cadastro`)VALUES('$caminho', '$usuario')") or die (mysqli_error($connection));
