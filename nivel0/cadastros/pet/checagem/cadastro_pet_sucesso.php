...
$sql_ref = mysql_query("SELECT * FROM `tab_temp_pet` WHERE user_cadastro='$usuario'") or die("erro ao selecionar1");
...
$sql_excl_orig = mysql_query("SELECT * FROM `tab_pet` WHERE codigo='$rad_sel_visl'") or die("Erro ao selecionar   -   Selecionar User_cadastro inicial  sql_excl");
...
$sql_atlzfoto = mysql_query("UPDATE `tab_pet` SET caminho_foto='$novo_caminho_foto' WHERE codigo='$rad_sel_visl'") or die (mysql_error());
}
}// fecha if
// FINAL DA INSTRUÇÃO (COPIA A NOVA FOTO PARA LOCAL "UPLOADS")

//APAGA FOTO TEMPORÁRIA TAB_TEMP_CLIE
$sql_ref3= mysql_query("SELECT * FROM `tab_temp_pet` WHERE user_cadastro='$usuario'") or die("erro ao selecionar1");
...
$sql_ref1 = mysql_query("SELECT * FROM `tab_pet` WHERE codigo='$rad_sel_visl'") or die("erro ao selecionar2");

if ($linha_ref1 = mysql_fetch_array($sql_ref1)) {

// ATUALIZA OS DADOS
$sql1 = mysql_query("UPDATE `tab_pet` SET nome='$txt_nome_pet', sexo='$txt_sexo', cor='$txt_cor', dono='$txt_dono', cod_dono='$txt_cod_dono', data_nasc ='$txt_data_nasc', especie='$txt_especie', raca ='$txt_raca', porte ='$txt_porte', mensalista = '$txt_mensal', chip='$txt_chip', rga='$txt_rga', obs= '$txt_obs_pet', data_cadastro  ='$data_atual' WHERE codigo='$rad_sel_visl'") or die (mysql_error());

}else{

// CASO NÃO EXISTA INSERE O NOVO CLIENTE
$sql2 = mysql_query("INSERT INTO `tab_pet` (`codigo`, `nome`, `sexo`,`cor`, `dono`, `cod_dono`, `data_nasc`, `especie`, `raca`, `porte`, `mensalista`, `chip`, `rga`, `obs`, `caminho_foto`, `user_cadastro`, `data_cadastro`) VALUES (NULL, '$txt_nome_pet', '$txt_sexo', '$txt_cor',
'$txt_dono', '$txt_cod_dono', '$txt_data_nasc', '$txt_especie', '$txt_raca', '$txt_porte', '$txt_mensal', '$txt_chip', '$txt_rga', '$txt_obs_pet', '$novo_caminho_foto', '$usuario', '$data_atual')") or die (mysql_error());
}
