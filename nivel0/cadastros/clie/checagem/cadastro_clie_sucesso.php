...
$sql_excl_orig = mysql_query("SELECT * FROM `tab_clie` WHERE codigo='$rad_sel_visl'") or die("Erro ao selecionar   -   Selecionar User_cadastro inicial  sql_excl");
...
$sql_atlzfoto = mysql_query("UPDATE `tab_clie` SET caminho_foto='$novo_caminho_foto' WHERE codigo='$rad_sel_visl'") or die (mysql_error());
}
}// fecha if
...
$sql2 = mysql_query("INSERT INTO `tab_clie` (`codigo`, `nome`, `sexo`, `endereco`, `cep`, `bairro`, `cidade`, `uf`, `ddd_tel`, `tel`, `ddd_cel`, `cel`, `rg`, `cpf`, `data_nasc`, `obs`, `caminho_foto`, `user_cadastro`, `data_cadastro`) VALUES (NULL, '$txt_nome_clie', '$sel_sexo', '$txt_end_clie', '$txt_cep_clie', '$txt_bairro_clie', '$txt_cidade_clie', '$txt_uf', '$txt_ddd_tel_clie', '$txt_tel_clie', '$txt_ddd_cel_clie', '$txt_cel_clie', '$txt_rg_clie', '$txt_cpf', '$txt_data_nasc_clie', '$txt_obs_clie', '$novo_caminho_foto', '$usuario','$data_atual')") or die (mysql_error());
}
...
$sql_ref2 = mysql_query("SELECT * FROM `tab_clie` WHERE nome='$txt_nome_clie'") or die("erro ao selecionar - sql_ref2");
	if ($linha_ref2 = mysql_fetch_array($sql_ref2)) {
