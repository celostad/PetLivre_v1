...
$sql_excl = mysqli_query($connection, "SELECT * FROM `tab_temp_fornecedor` WHERE user_cadastro='$usuario'") or die("Erro ao selecionar   -   Selecionar User_cadastro inicial  SQL");
...
$sql_1 = mysqli_query($connection, "SELECT * FROM `tab_temp_fornecedor` WHERE user_cadastro='$usuario'") or die("erro ao selecionar1 - TELA UPLOAD");

if ($linha = mysqli_fetch_array($sql_1)){

$sql_2 = mysqli_query($connection, "UPDATE `tab_temp_fornecedor` SET caminho_foto ='$caminho' WHERE user_cadastro='$usuario'") or die (mysqli_error($connection));

}else{

$sql_3 = mysqli_query($connection, "INSERT INTO `tab_temp_fornecedor` (`caminho_foto`, `user_cadastro`)VALUES('$caminho', '$usuario')") or die (mysqli_error($connection));
...
window.opener.document.form.action="checagem/grava_variaveis_bd.php";
window.opener.location = "../cad_pet.php";
window.opener.document.form.submit();
