...
$sql_ref = mysqli_query($connection, "SELECT * FROM `tab_temp_fornecedor` WHERE user_cadastro='$usuario'") or die("erro ao selecionar1");
...
$sql_excl_orig = mysqli_query($connection, "SELECT * FROM `tab_fornecedor` WHERE codigo='$rad_sel_visl'") or die("Erro ao selecionar   -   Selecionar User_cadastro inicial  sql_excl");
...
$sql_atlzfoto = mysqli_query($connection, "UPDATE `tab_fornecedor` SET caminho_foto='$novo_caminho_foto' WHERE codigo='$rad_sel_visl'") or die (mysqli_error($connection));
}
}// fecha if
// FINAL DA INSTRUÇÃO (COPIA A NOVA FOTO PARA LOCAL "UPLOADS")

//APAGA FOTO TEMPORÁRIA TAB_TEMP_FORNECEDOR
$sql_ref3= mysqli_query($connection, "SELECT * FROM `tab_temp_fornecedor` WHERE user_cadastro='$usuario'") or die("erro ao selecionar1");
...
$sql_ref1 = mysqli_query($connection, "SELECT * FROM `tab_fornecedor` WHERE codigo='$rad_sel_visl'") or die("erro ao selecionar2");

if ($linha_ref1 = mysqli_fetch_array($sql_ref1)) {

// ATUALIZA OS DADOS
$sql1 = mysqli_query($connection, "UPDATE `tab_fornecedor` SET razao_social='$txt_razao_social', contato='$txt_contato', cnpj='$txt_cnpj', endereco='$txt_endereco', bairro='$txt_bairro', cidade ='$txt_cidade', cep='$txt_cep', uf ='$txt_uf', ddd_tel ='$txt_ddd_tel',
tel_com = '$txt_tel_com', email='$txt_email', ddd_cel='$txt_ddd_cel', cel= '$txt_cel', obs_forne  ='$txt_obs_forne', data_ult_atlz ='$data_atual' WHERE codigo='$rad_sel_visl'") or die (mysqli_error($connection));

}else{

// CASO NÃO EXISTA INSERE O NOVO CLIENTE
$sql2 = mysqli_query($connection, "INSERT INTO `tab_fornecedor` (`codigo`, `razao_social`, `contato`,`cnpj`, `endereco`, `bairro`, `cidade`, `cep`, `uf`, `ddd_tel`, `tel_com`, `email`, `ddd_cel`, `cel`, `obs_forne`, `caminho_foto`, `user_cadastro`, `data_cadastro`) VALUES (NULL, '$txt_razao_social', '$txt_contato', '$txt_cnpj', '$txt_endereco', '$txt_bairro', '$txt_cidade', '$txt_cep', '$txt_uf', '$txt_ddd_tel', '$txt_tel_com', '$txt_email', '$txt_ddd_cel', '$txt_cel', '$txt_obs_forne', '$novo_caminho_foto', '$usuario', '$data_atual')") or die (mysqli_error($connection));
}
