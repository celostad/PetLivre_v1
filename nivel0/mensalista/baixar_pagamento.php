...
$sql_produto = mysql_query("SELECT * FROM tab_produto WHERE codigo=6");
if($linha_produto = mysql_fetch_array($sql_produto)){
...
if (!empty($txt_cod_pet)){
    ...
    $sql = mysql_query("SELECT * FROM tab_mensalista WHERE cod_pet='$txt_cod_pet'");
		if($linha = mysql_fetch_array($sql)){
		
			$sql1 = mysql_query("UPDATE tab_mensalista set status=1, user_pag='$usuario', data_pag='$data_atual' WHERE cod_pet='$txt_cod_pet' && month(`data_banho`)= '$mes_num'");
		
		
			$sql2 = mysql_query("INSERT INTO `tab_caixa` (`codigo`, `cod_produto`, `produto`, `pet`, `qtde`, `medida`,`valor`, `especie`, `obs`, 
